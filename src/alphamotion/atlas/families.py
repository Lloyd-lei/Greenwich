...
FAM_RE = [
    ("jacks", re.compile(r"jumping[_ ]?jack", re.I)),
    ("run",   re.compile(r"(?<![a-z])run|(?<![a-z])jog|sprint", re.I)),
    ...
    ("jump",  re.compile(r"jump|(?<![a-z])hop|leap", re.I)),
    ("stand", re.compile(r"(?<![a-z])stand|still|idle|posing|t_?pose", re.I)),
    ("sit",   re.compile(r"(?<![a-z])sit|chair|crouch|squat", re.I)),
...
def family_id(name: str) -> int:
    return FAMILIES.index(name if name in FAMILIES else family_of(name))
